...
using BrevTools.AlbionEventHandlers;
using BrevTools;
...
namespace BraveClient.network
{
    public class PacketParser : PhotonParser
    {
        ...
        private SayChatHandler _newSayChatHandler = new SayChatHandler();
        private PlayerInformationHandler _newPlayerInformationHandler = new PlayerInformationHandler();
        ...
        private async void DebugPacket(short rawCode, PacketType packetType, Dictionary<byte, object> rawData)
        {
            ...
            {
                case "SayChat":
                    await SayChatEventHandlerAsync(rawData);
                    break;
                case "PlayerInformation":
                    await PlayerInformationEventHandlerAsync(rawData);
                    break;
                default:
                    return;
        ...
        private async Task SayChatEventHandlerAsync(Dictionary<byte, object> paramaters)
        {
            var value = new SayChatEvent(paramaters);
            await _newSayChatHandler.OnActionAsync(value);
        }

        private async Task PlayerInformationEventHandlerAsync(Dictionary<byte, object> parameters)
        {
            var value = new PlayerInformationEvent(parameters);
            await _newPlayerInformationHandler.OnActionAsync(value);
        }
